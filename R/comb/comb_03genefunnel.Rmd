...
results <- rbind(
  gse_list[[1]], gse_list[[2]], gse_list[[3]], gse_list[[4]],
  gse_list[[5]], gse_list[[6]], gse_list[[7]]
)
gse_corrections <- vector("list", length = 5)
idx <- 13
for (i in seq_along(gse_corrections)) {
  gse_corrections[[i]] <- p.adjust(results[ , idx], method = "BH")
  idx <- idx + 1
}
results <- rbind(
  deg_list[[1]], deg_list[[2]], deg_list[[3]], deg_list[[4]],
  deg_list[[5]], deg_list[[6]], deg_list[[7]]
)
deg_corrections <- vector("list", length = 5)
idx <- 13
for (i in seq_along(deg_corrections)) {
  deg_corrections[[i]] <- p.adjust(results[ , idx], method = "BH")
  idx <- idx + 1
}
```
...
for (i in seq_along(gse_corrections)) {
  results[ , idx] <- gse_corrections[[i]][length]
  idx <- idx + 1
}
...
length <- length1:length2
for (i in seq_along(deg_corrections)) {
  results[ , idx] <- deg_corrections[[i]][length]
  idx <- idx + 1
}
