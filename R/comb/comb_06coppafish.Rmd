...
# Read data.
# ----------
seurat_orig <- readRDS(file.path(cache_dir, "annotated_neuronal.rds"))
...
```{r}
seurat <- seurat_orig[
  , grep("Glutamatergic", x = seurat_orig$subcluster_annotations)
]
seurat$subcluster_annotations <- factor(seurat$subcluster_annotations)
...
```{r}
seurat <- seurat_orig[
  , grep("GABAergic", x = seurat_orig$subcluster_annotations)
]
seurat$subcluster_annotations <- factor(seurat$subcluster_annotations)
