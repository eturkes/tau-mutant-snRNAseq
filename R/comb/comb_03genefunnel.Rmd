...
for (i in seq_along(seurat_list)) {
  ...
  thresh <- min(table(gse$genotype_batch))

  keep <- filterByExpr(
    gse, group = gse$genotype,
    min.count = thresh, min.total.count = thresh
  )
  gse <- gse[keep, ]
  ...
  logcounts(gse, withDimnames = FALSE) <- cpm(
    dge, log = TRUE, prior.count = sqrt(sqrt(thresh))
  )

  gse <- gse[rownames(gse) %in% names(gene_sets_keep), ]

  keep <- filterByExpr(
    deg, group = deg$genotype,
    min.count = thresh / 10, min.total.count = thresh / 10
  )
  deg <- deg[keep, ]
  ...
  logcounts(deg, withDimnames = FALSE) <- cpm(
    dge, log = TRUE, prior.count = sqrt(sqrt(sqrt(thresh)))
  )
