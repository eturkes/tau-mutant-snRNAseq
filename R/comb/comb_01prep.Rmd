...
We add back per-sample SCTransform data and perform `PrepSCTFindMarkers()` to make samples comparable for differential expression.
...
if (file.exists(rds)) {
  ...
  sct_seurat[["RNA"]] <- CreateAssay5Object(GetAssayData(seurat, assay = "RNA"))
  sct_seurat <- PrepSCTFindMarkers(sct_seurat)
  seurat[["SCT_NORM"]] <- CreateAssay5Object(GetAssayData(sct_seurat))
...
VlnPlot(seurat, features = "nCount_SCT", group.by = "genotype_batch")
VlnPlot(seurat, features = "nCount_SCT_NORM", group.by = "genotype_batch")
```
