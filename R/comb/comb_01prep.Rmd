...
names(metadata_to_plot) <- c(
  "broad_annotations", "allen_labels", "seurat_clusters", "genotype", "batch"
)
...
if (file.exists(rds)) {
  ...
  remove <- grep("batch01/NLGF_MAPTKI|batch01/NLGF_P301S", x = file_list)
...
if (file.exists(rds)) {
  ...
  seurat <- SCTransform(
    seurat, vst.flavor = "v2", vars.to.regress = c("batch", "sex"),
  ...
  gene_anno <- data.frame(
    geneids = seurat@misc$geneids,
    ensembl = rownames(GetAssayData(seurat, assay = "RNA"))
  )
  gene_anno <- gene_anno[gene_anno$ensembl %in% rownames(seurat_geneids), ]
  rownames(seurat_geneids) <- gene_anno$geneids
  rm(gene_anno)
