...
if (file.exists(rds)) {
  ...
  file_list <- file_list[grep("seurat_annotated.rds", file_list)]
  ...
  seurat$allen_labels <- factor(seurat$allen_labels)
...
Add in raw counts and gene symbols.
...
if (file.exists(rds)) {
  ...
  seurat[["RNA"]] <- CreateAssay5Object(
    GetAssayData(seurat_raw, layer = "counts")
  )
  rm(seurat_raw)

  geneids <- read.delim(file.path(data_dir, "features.tsv.gz"), header = FALSE)
  geneids <- geneids[geneids$V1 %in% rownames(seurat[["RNA"]]), ]
  dup <- geneids[
