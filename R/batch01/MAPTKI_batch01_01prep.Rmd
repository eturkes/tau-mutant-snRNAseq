...
contam_genes <- names(contam)[which(contam == 1)]
contam_genes <- gene_anno[gene_anno$ensembl_gene_id %in% contam_genes, ]
remove <- which(rownames(seurat) %in% contam_genes$ensembl_gene_id)
seurat <- seurat[-remove, ]
contam <- contam[-remove]
seurat[["RNA"]][[]] <- data.frame(percent_contamination = contam)
rm(contam)
seurat
datatable_download(contam_genes)
...
seurat <- subset(seurat, subset = nFeature_RNA >= 175 & nCount_RNA >= 200)
seurat <- seurat[which(rowSums(GetAssayData(seurat)) > 0), ]
seurat
