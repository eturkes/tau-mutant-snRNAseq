...
sel <- c()
sig_mat_sub <- sig_mat
cols <- seq(5, ncol(sig_mat_sub))
while (length(sel) < 30) {
  tmp <- unique(sig_mat_sub)
  tmp <- tmp[!apply(tmp[, cols, drop = FALSE], 1, function(x) all(x == x[1])), ]

  sel <- c(sel, rownames(tmp))
...
fluidRow(
  ...
  column(
    width = 1,
    actionButton(
      paste0("clear_deg", idx), label = "Clear Genes", class = "btn-primary"
    ),
...
selectizeInput(
  ...
  selected = rownames(sig_mat)[rownames(sig_mat) %in% sel],
  multiple = TRUE,
  width = "100%"
)
    ...
    draw(
      Heatmap(
        ...
        cell_fun = function(j, i, x, y, width, height, fill) {
          grid.text(
            sig_tmp[i, j], x = x, y = y,
            gp = gpar(fontsize = (-0.25 * nrow(mat)) + 17.5)
          )
        },
