...
if (file.exists(rds)) {
  ...
  gene_sets_keep <- gene_sets_keep[gene_sets_keep <= 0.5]
...
gse <- gse[rowSums(counts(gse) > 0) == ncol(gse), ]

dge <- DGEList(counts(gse))
dge <- calcNormFactors(dge)
logcounts(gse, withDimnames = FALSE) <- cpm(dge, log = TRUE)
gse
...
deg <- deg[rowSums(counts(deg) > 0) == ncol(deg), ]

dge <- DGEList(counts(deg))
dge <- calcNormFactors(dge)
logcounts(deg, withDimnames = FALSE) <- cpm(dge, log = TRUE, prior.count = 4)
deg
```
...
keep <- filterByExpr(
  keep, group = factor(keep$genotype), min.total.count = 25, min.count = 25
)
cont_fit <- cont_fit[keep, ]
...
mat <- logcounts(gse)[
  rownames(gse) %in% rownames(gse_results_up)[1:30], , drop = FALSE
]
mat <- mat[
...
mat <- logcounts(gse)[
  rownames(gse) %in% rownames(gse_results_down)[1:30], , drop = FALSE
]
mat <- mat[
...
cont_mat <- makeContrasts(
  paste(
    paste0("deg.genotype", "S305N"),
    paste0("deg.genotype", "MAPTKI"),
    sep = "-"
  ),
  levels = design
)
...
keep <- deg[ , deg$genotype %in% genotype]
keep <- filterByExpr(
  keep, group = factor(keep$genotype), min.total.count = 15, min.count = 15
)
cont_fit <- cont_fit[keep, ]
...
mat <- logcounts(deg)[
  rownames(deg) %in% rownames(deg_results_up)[1:30], , drop = FALSE
]
mat <- mat[
  match(rownames(deg_results_up)[1:30], rownames(mat)), , drop = FALSE
]
rownames(mat) <- deg_results_up$gene[1:30]
mat <- removeBatchEffect(mat, batch = deg$batch, group = deg$genotype)
Heatmap(
  mat,
...
mat <- logcounts(deg)[
  rownames(deg) %in% rownames(deg_results_down)[1:30], , drop = FALSE
]
mat <- mat[
...
cont_mat <- makeContrasts(
  paste(
    paste0("deg.genotype", "NLGF_MAPTKI"),
    paste0("deg.genotype", "MAPTKI"),
    sep = "-"
  ),
  levels = design
...
cont_mat <- makeContrasts(
  paste(
    paste0("deg.genotype", "NLGF_S305N"),
    paste0("deg.genotype", "S305N"),
    sep = "-"
  ),
  levels = design
...
cont_mat <- makeContrasts(
  paste(
    paste0("deg.genotype", "NLGF_P301S"),
    paste0("deg.genotype", "P301S"),
    sep = "-"
  ),
  levels = design
