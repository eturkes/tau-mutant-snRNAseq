...
for (i in seq_along(seurat_list)) {
  ...
  thresh <- min(table(seurat_list[[i]]$genotype_batch))

  gse <- as.SingleCellExperiment(seurat_list[[i]], assay = "GF")
  keep <- filterByExpr(
    counts(gse), group = gse$genotype, min.prop = 0.1,
    min.count = 1, min.total.count = 1
  )
  gse <- gse[keep, ]
  ...
  keep <- filterByExpr(
    counts(deg), group = deg$genotype, min.prop = 0.1,
    min.count = 1, min.total.count = 1
  )
  deg <- deg[keep, ]
  ...
  keep <- filterByExpr(
    ...
    min.count = 10, min.total.count = 10
  )
  dge <- dge[keep, ]
...
thresh <- min(table(seurat_list[[ctype_idx]]$genotype_batch))
thresh
